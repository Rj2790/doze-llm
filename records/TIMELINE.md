...
- 2026-09-12 11:02 IST — Vultr instance (sgp, 2×A16) destroyed after archive verified (tar 4.37 GB, 40 entries, byte count matches VM). Pending charges at teardown $89.50. No compute running anywhere.
...
- 2026-09-14 — User decision: stop and write up; open-source. `.env` found tracked in git since the Vultr deploy commits → purged from history (bundle backup off-repo), identifiers scrubbed, LICENSE / CITATION / README / REPORT written, adapters uploaded to HF (private). Force-push, credential rotation and visibility flip left to the user.
