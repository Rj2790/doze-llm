...
def _solution_with_wrong_steps(inst, break_steps=()):
...
def test_dreams_are_verified_against_solver_and_split():
    ...
    solves = [nr.gold_response(t_ok, "work"), _solution_with_wrong_steps(t_wrong, (3, 5, 7)),
              _solution_with_wrong_steps(t_near, (9,))]
    be = CannedBackend(proposals + solves)
    kept, stats = dreamer.dream(be, [seed_ex] * 3, n_variations=2, split=SPLIT, mode="work",
                                numbered=False, max_tokens=400)
    assert stats["generated"] == 6 and stats["tokens"] == sum(len(t.split()) for t in proposals + solves)
    assert [e.digits for e in kept] == [t_ok.digits, t_near.digits]
    assert all(e.source == "dream" and e.prefix == src.prefix for e in kept)
    assert stats["rejected"] == {"prefix_changed": 1, "unparsable": 1, "duplicate": 1, "wrong": 1}
...
def test_propose_prompt_mentions_seed_digits():
    seed_ex = filters.to_example(_ep(TRAIN, True, 11), "work", False)
    p = dreamer.propose_prompt(seed_ex)
    assert " ".join(TRAIN.digits) in p and "Digits:" in p and "12" in p and "last five" in p
...
def test_accepted_dreams_always_keep_source_prefix_and_report_structure():
    ...
    assert st["rejected"].get("prefix_changed", 0) == 0 and st["rejected"].get("duplicate", 0) == 0
...
def test_verbatim_copies_are_rejected_as_duplicate():
    src = SPLIT.train[11]
    seed_ex = filters.to_example(_ep(src, True, 11), "work", False)
    ...
    assert st["generated"] == 2 and st["kept"] == 1 and st["rejected"] == {"duplicate": 1}
    assert kept[0].digits == var.digits and kept[0].completion.strip() == nr.gold_response(var, "work")
    assert st["duplicates"] == 1 and st["accepted"][0]["duplicate"] is False


def test_decoupled_dreamer_solves_with_day_prompt_and_c3():
    """Step 1: digits-only proposal (temperature). Step 2: solve the new string
    with the ordinary day prompt (greedy), verified by the C3 keep rule."""
    src = SPLIT.train[12]
    seed_ex = filters.to_example(_ep(src, True, 11), "work", False)
    v1 = nr.Instance.from_digits(src.digits[:7] + "49111"); v2 = nr.Instance.from_digits(src.digits[:7] + "11149")
    bad_prefix = nr.Instance.from_digits(("1" if src.digits[0] != "1" else "4") + src.digits[1:])
    wrong_solution = _dream_with_steps_wrong(v2, [1, 4, 7])[0].split("\n", 1)[1]   # 3 wrong steps -> fails C3
    be = CannedBackend([nr.digits_line(v1.digits), nr.digits_line(bad_prefix.digits), nr.digits_line(v2.digits), "Digits: 1 2",
                        nr.gold_response(v1, "work"), wrong_solution])
    kept, st = dreamer.dream(be, [seed_ex] * 2, n_variations=2, split=SPLIT, mode="work", numbered=False, max_tokens=400)
    assert st["generated"] == 4 and st["kept"] == 1 and kept[0].digits == v1.digits
    assert st["rejected"] == {"prefix_changed": 1, "unparsable": 1, "wrong": 1}
    # the solve prompts are exactly the day prompts of the proposed strings
    solve_prompts = [p for p in be.prompts if p.endswith(" ".join(v1.digits)) or p.endswith(" ".join(v2.digits))]
    assert solve_prompts and all(p == nr.format_prompt(nr.Instance.from_digits("".join(nr.parse_digits_line(p))), "work") for p in solve_prompts)
    assert st["tokens"] == sum(len(t.split()) for t in be.prompts_answered_texts) if hasattr(be, "prompts_answered_texts") else st["tokens"] > 0


def test_digits_proposal_prompt_is_short_and_instructs_last_five():
    seed_ex = filters.to_example(_ep(TRAIN, True, 11), "work", False)
    p = dreamer.propose_prompt(seed_ex)
